...
fn print_benchmark_data(out: anytype, data: BenchmarkData) !void {
    if (data.logic) |logic|
    ...
    if (data.generatedOn) |generatedOn|
    ...
    if (data.subbenchmarkCount != 0)
        try out.print("Subbenchmark count: {}\n", .{data.subbenchmarkCount});
...
pub fn main() !u8 {
                ...
                .set_info => {
                    ...
                    if (AttrMap.get(ptr[attrSpan.start..attrSpan.end])) |attr| {
                        switch (attr) {
                            ...
                            .source => {
                                if (get_string(ptr, attrSpan.end)) |x|
                                    benchmarkData.description = ptr[x.start..x.end];
                            },
                        }
                    }
    ...
    try print_benchmark_data(stdout, benchmarkData);
    try bw.flush();
    return 0;
}
